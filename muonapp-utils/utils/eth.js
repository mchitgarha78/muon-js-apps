...
function getWeb3(network) {
  ...
  if (!web3Instances[chainId]) {
    ...
    web3Instances[chainId] = new Web3(
      new HttpProvider(EthRpcList[chainId][nextRpc]),
    );
  }
...
function getWeb3Sync(network) {
  ...
  if (!web3Instances[chainId]) {
    const nextRpc =
      ((lastUsedRpcIndex[chainId] ?? -1) + 1) % EthRpcList[chainId].length;
    lastUsedRpcIndex[chainId] = nextRpc;
    web3Instances[chainId] = new Web3(
      new HttpProvider(EthRpcList[chainId][nextRpc]),
    );
...
function getTransaction(txHash, network) {
  return getWeb3(network).then((web3) =>
    wrappedCall(network, web3.eth.getTransaction.bind(web3), [txHash]),
  );
}

function getTransactionReceipt(txHash, network) {
  return getWeb3(network).then((web3) =>
    wrappedCall(network, web3.eth.getTransactionReceipt.bind(web3), [txHash]),
  );
}
...
function getBlock(network, blockHashOrBlockNumber) {
  return getWeb3(network).then((web3) => {
    return wrappedCall(network, web3.eth.getBlock.bind(web3), [
...
function getPastEvents(network, contractAddress, abi, event, options) {
  return getWeb3(network).then((web3) => {
    let contract = new web3.eth.Contract(abi, contractAddress);
    return wrappedCall(network, contract.getPastEvents.bind(contract), [
      event,
      options,
    ]);
  });
